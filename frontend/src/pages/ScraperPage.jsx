...
const ScraperPage = () => {
    ...
    useGSAP(() => {
        ...
        let split = SplitText.create(`#split${logMessage.length}`, { type: "words, lines" });
    ...
    const handleSubmit = async () => {
        ...
        if (repeat) {
            ...
            data = new URLSearchParams({
                time: `${scheduleDate}T${scheduleTime.hour.padStart(2, 0)}:${scheduleTime.minute.padStart(2, 0)}:${scheduleTime.second.padStart(2, 0)}`,
                source_name: selectedWebsite.name,
                random: false
            });
        }
        ...
        <section className='w-full min-h-screen h-full flex flex-col items-center justify-center'>
            ...
            <div className="mockup-browser border-base-300 border w-2/4 mt-12">
                <div className="mockup-browser-toolbar">
                    ...
                    {isLoading && schedule &&
                        <Countdown
                            targetTime={`${scheduleDate}T${scheduleTime.hour.padStart(2, 0)}:${scheduleTime.minute.padStart(2, 0)}:${scheduleTime.second.padStart(2, 0)}`}
                        />
                    }
                </div>
                    ...
                    <div className="bg-neutral-800 size-full mt-4 mr-3 rounded-lg text-white overflow-auto py-6">
                        {logMessage.length > 0 ?
                            logMessage?.map((message, index) => (
                                <span key={index}>
                                    <div className="text-green-500 ml-3 mt-2" id={`split${index + 1}`}>
                                        {message}
                                    </div>
                            ...
                            <>
                                <div className="flex items-center justify-around gap-2">
                                    <input
                                        ...
                                        onChange={(e) => setRepeatTime({ ...repeatTime, value: e.target.value })}
                                    />
                                    <select
                                        defaultValue="Pick a color"
                                        className="select border-neutral-400 bg-white"
                                        onChange={(e) => setRepeatTime({ ...repeatTime, type: e.target.value })}
                                    >
...

                        {
                            ...
                            <>
                                ...
                                <div popover='auto' id="cally-popover1" className="dropdown bg-white rounded-box shadow-lg ">
                                    <calendar-date className="cally" onchange={(e) => {
                                ...
                                <div className="flex items-center justify-center gap-2 mt-2">
                                    ...
                                    <input
                                        ...
                                        className='input bg-white border-neutral-400'
                                        onChange={(e) => setScheduleTime({ ...scheduleTime, second: `${e.target.value}` })}
                                    />
                                </div>
                            </>
                        }
            ...
            <div className="mb-12 mt-8 flex justify-center items-center gap-3 transition-all">
                <button className={`btn btn-wide btn-primary btn-xl disabled:!bg-primary/90`} disabled={isLoading} onClick={() => handleSubmit()}>
                    {isLoading ?
                        <>
                ...
                {isLoading &&
                    <button className="btn size-14 bg-red-500 border-none" onClick={() => handleCancel()}>
                        <div className="bg-white size-6 rounded-sm"></div>
                    </button>
                }
            </div>
            <div className="h-36 w-full">
                <LogoCarousel selectedWebsite={selectedWebsite} setSelectedWebsite={setSelectedWebsite} />
            </div>
        </section>
    )
}
